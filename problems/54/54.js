...
const parseRank = (raw) => parseInt(raw, 10) || ranks[raw];

const last = (items) => items[items.length - 1];

const descending = (a, b) => b - a;
const sortHand = hand => hand.sort((a, b) => descending(a.rank, b.rank))

const parse = (contents) => {
  const lines = contents.split("\n");
  const hands = lines.map((line) => {
    const cards = line.split(" ").map(([rawRank, suit]) => ({
      rank: parseRank(rawRank),
      suit,
    }));
    const left = sortHand(cards.slice(0, 5))
    const right = sortHand(cards.slice(5))
...
const isFlush = ([first, ...rest]) =>
  rest.every((card) => card.suit === first.suit);

const isStandardStraight = (hand) => {
  const [, ...rest] = hand;
  return rest.every((card, i) => card.rank === hand[i].rank - 1);
};
const isWheel = ([first, ...rest]) =>
  first.rank === ranks.A && isStandardStraight(rest);
const isStraight = (hand) => isStandardStraight(hand) || isWheel(hand);

const buildGroups = (hand) => {
  const groups = [];

  hand.forEach(({ rank }) => {
    const lastGroup = last(groups);

    if (lastGroup?.rank === rank) {
      lastGroup.size++;
    } else {
      groups.push({ rank, size: 1 });
    }
  });

  // Sort by group size, breaking ties with rank.
  groups.sort(
    (a, b) => descending(a.size, b.size) || descending(a.rank, b.rank)
  );
  return groups;
};
...
const types = {
  ...
  quads: 7,
  straightFlush: 8,
  // A royal flush is not a separate type of hand; it is simply the highest straight flush.
};
...
const getHandRank = (hand) => {
  ...
  // Tie breakers between straights are resolved by the low card since the high card is unreliable with wheels.
  if (flush && straight) return [types.straightFlush, lowCard(hand)];

  const groups = buildGroups(hand);
  const groupRanks = groups.map((g) => g.rank);
  const largestGroupSize = groups[0].size;
  const secondGroupSize = groups[1]?.size;

  if (largestGroupSize === 4) return [types.quads, ...groupRanks];
  if (largestGroupSize === 3 && secondGroupSize === 2)
    return [types.fullHouse, ...groupRanks];
  if (flush) return [types.flush, ...groupRanks];
  if (straight) return [types.straight, lowCard(hand)];
  if (largestGroupSize === 3) return [types.trips, ...groupRanks];
  if (largestGroupSize === 2)
    return [secondGroupSize === 2 ? types.twoPair : types.pair, ...groupRanks];
  return [types.highCard, ...groupRanks];
};

const zip = (as, bs) => as.map((a, i) => [a, bs[i]]);

const doesPlayerOneWin = ([left, right]) => {
  const zippedRanks = zip(getHandRank(left), getHandRank(right));
  const [l, r] = zippedRanks.find(([l, r]) => l !== r);
  return l > r;
};

const main = (fileContents) => {
  const hands = parse(fileContents);

  return hands.filter(doesPlayerOneWin).length;
};
